package newsaggregator.controller.dashboard.trend.trendPackage.timeDisplayer;
...
public class HalfYearRange implements TimeDisplay{
    ...
    public PairArray timeRangeGrouping(int yearSpan) {
		...
		lastDate = lastDate.minusMonths(lastDate.getMonthValue()-1);
		...
		newDateList.add("Before ".concat(othersDate), 0); //Để biểu diễn các năm nằm ngoài yearSpan
		...
		for(Pair date: this.dateList) {
			...
			if(dateString.substring(0, 4).compareTo(othersDate) < 0)
				dateString = "Before ".concat(othersDate);
